...
const Track = ({ track, artistId }: { track: Track; artistId?: number }) => {
  ...
  return (
    <Link href={`/track-details/${artistId}`}>
      {track && (
        <Box width={['100%', '50%', '33%']} p={2} key={track.id} boxShadow={theme.shadows.cards} mb={3}>
          <Box
            css={css`
              cursor: pointer;
            `}
          ...
          <Flex justifyContent="space-between" px={1}>
            ...
            {track.duration && <Text fontSize={[2, 2]}>{secToMinutes(track.duration)}</Text>}
          </Flex>
          <Text fontWeight={300} px={1}>{`By ${track.artist?.name}`}</Text>
          <Text fontWeight={400} px={1} fontSize={[2, 2]}>
            {track.album?.title}
          </Text>
        </Box>
      )}
